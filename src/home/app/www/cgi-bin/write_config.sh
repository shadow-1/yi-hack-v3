...
printf "PROXYCHAINSNG=$FORM_PROXYCHAINSNG\nHTTPD=$FORM_HTTPD\nTELNETD=$FORM_TELNETD\nFTPD=$FORM_FTPD" > "$CONF_FILE"
...
<head>
	...
	<meta http-equiv="refresh" content="3;url=/cgi-bin/read_config.sh" />

EOM1

echo "	<title>Yi Dome 1080p</title>"
...
if [[ $VAR_HTTPD == "yes" ]] ; then
	echo "					<input type="radio" name="HTTPD" id="LIGHTTPD" value="yes" checked="checked"> Yes"
else
	echo "					<input type="radio" name="HTTPD" id="LIGHTTPD" value="yes"> Yes"
fi
...
if [[ $VAR_HTTPD == "no" ]] ; then
	echo "					<input type="radio" name="HTTPD" id="LIGHTTPD" value="no" checked="checked"> No"
else
	echo "					<input type="radio" name="HTTPD" id="LIGHTTPD" value="no"> No"
fi
